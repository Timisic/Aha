...
async function pipelineRecall(args, sourceText) {
  ...
  const { queryResults, warnings: queryWarnings, errors } = await runQmdPlanQueries(args, plan.queries);
  ...
  const warnings = [
    `Query plan generated by ${plan.query_generated_by}${plan.query_generation_fallback ? ` after fallback: ${plan.query_generation_error}` : ""}.`,
    "Retrieval kept one deterministic source-note query after the model-generated rewrites.",
    relationJudge.ok
  ...
  const summary = `${plannerName} generated ${plan.model_query_count} QMD query rewrites plus 1 deterministic source-note fallback; mixed retrieval returned ${candidates.length} reranked candidates; Relation Judge reviewed ${relationJudge.reviewedCount} candidate excerpts.`;
