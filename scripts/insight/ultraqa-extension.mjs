...
writeFileSync(
  ...
  [
    ...
    "const queryIndex = args.findIndex((arg) => arg === 'search' || arg === 'vsearch' || arg === 'query') + 1;",
    ...
    "    { file: 'note/feedback-visible-gap.md', title: 'example-feedback-density', chunk_text: 'Example fixture note text for feedback density.。', score: 0.8, source_id: 'obsidian', query },",
    "    { file: 'note/feedback-loop-source.md', title: '反馈迭代的来源动力', chunk_text: '输出带来反馈，但也要防止确认偏差。', score: 0.7, source_id: 'obsidian', query }",
    "  ]));",
    "}",
    "",
  ].join("\n"),
...
try {
  ...
  const searchResult = await normal.tools.get("insight_search_memory").execute(
    "search-normal",
    {
      queries: [
        ...
        { text: "学习系统 反馈 成长", kind: "open-ended" },
  ...
  const compactGrillMessages = await normal.emitContext([
    ...
    { role: "tool", content: "gbrain trace noise", timestamp: Date.now() - 1 },
