...
- QMD plan queries 逐条执行，避免多个 `qmd query` 进程争用 QMD/SQLite runtime；单条默认 30 秒超时，并给 QMD 传 `-C 20` 限制内部 rerank 候选数。慢查询会出现在 warning 中，避免旧版多条 120 秒 timeout 累积；wrapper 不会自动降级到 `qmd vsearch`。
