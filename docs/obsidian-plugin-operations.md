...
- CLI 批量写入的是指定 plugin id 的 Session Store；必须与随后打开的插件一致（本机当前为 `aha-memory-surface-dev`）。执行期间停用该插件，避免后台插件覆盖批量结果，但保留 Obsidian 运行，供链接/反链 CLI 查询使用；完成后重新启用/重载，再打开对应源笔记的 Aha Panel，无需手动导入。
...
- 批量入口显式传递 `obsidianCommand`（默认 `obsidian`）到共享 core；否则图扩展会以 undefined 命令失败，但仍可能返回 `ok:true`。批量验收必须检查图扩展 warning，不能只看成功条数。
