...
export function buildMemoryReviewPrompt(active: ActiveSession): string {
  ...
  return [
    ...
    "2. Ask the user to choose exactly one next move: search more memory, or enter grill.",
    "3. If the user asks to search more, call `insight_search_memory` again and merge the new candidates.",
    "4. If the user explicitly chooses grill, call `insight_update_state` with exact stage enum `review_grill` (not `grill`, `questioning`, or `grill-review`).",
    "5. Do not ask grill questions while still in memory_review; if the stage update fails, stay in memory_review and fix the transition first.",
    "6. If you need to open a candidate note, use the QMD file path when available; otherwise use Obsidian CLI or locate the exact file path first.",
    "7. Do not call `insight_update_state` only to restate the current stage.",
    "8. When you need exact Obsidian note identity, source-note content, backlinks, outlinks, or title/alias resolution, use Obsidian CLI.",
  ].join("\n");
}
...
export function buildReviewGrillPrompt(active: ActiveSession): string {
  ...
  return [
    ...
    "5. Use Obsidian CLI when exact note identity, source-note content, backlinks, outlinks, or title/alias resolution matters.",
    "",
...
export function buildResumePrompt(active: ActiveSession): string {
  ...
  return [
    ...
    "- memory_review: show the candidate table and ask whether to search more memory or enter grill.",
    "- review_grill: use the Review-Grill loop; do not call insight_update_state only to restate the current stage.",
    "- summary: continue only if the user asks to revise or save a summary draft.",
